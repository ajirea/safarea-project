...
$route->get('/recent-order/{dropshipper_id}', function (Request $request, Response $response, $args) {
    ...
    $query = $this->get('db')->prepare("SELECT A.*, B.thumbnail, B.name, C.name AS buyer_name, C.phone FROM orders AS A INNER JOIN products AS B ON B.id=A.product_id INNER JOIN buyers AS C ON C.id=A.buyer_id WHERE A.user_id=? ORDER BY created_at DESC LIMIT 7");
    $query->bindParam(1, $args['dropshipper_id']);
    $query->execute();
